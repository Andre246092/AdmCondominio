...
// Renderiza conteúdo específico de cada seção
switch(id) {
case "apartamentos":
Apartamentos.render();
break;
...
case "config":
// Se tiver objeto de Configurações para renderizar campos de senha e parâmetros
if (typeof Configuracoes !== "undefined" && Configuracoes.render) {
Configuracoes.render();
}
break;

default:
// Qualquer outra seção não precisa de ação
break;
}
},
...
const Config = {
  ...
  loadParametros() {
    ...
    document.getElementById("taxaMinima").value =
      monthData.parametros.agua.taxaMinima;

    document.getElementById("limiteMinimo").value =
      monthData.parametros.agua.limiteMinimo;

    document.getElementById("valorExcedente").value =
      monthData.parametros.agua.valorExcedente;

    document.getElementById("valorGas").value =
      monthData.parametros.gas.valorM3;
  ...
  saveParametros() {
    ...
    data[monthKey].parametros.agua.taxaMinima =
      parseFloat(document.getElementById("taxaMinima").value);

    data[monthKey].parametros.agua.limiteMinimo =
      parseFloat(document.getElementById("limiteMinimo").value);

    data[monthKey].parametros.agua.valorExcedente =
      parseFloat(document.getElementById("valorExcedente").value);

    data[monthKey].parametros.gas.valorM3 =
      parseFloat(document.getElementById("valorGas").value);
